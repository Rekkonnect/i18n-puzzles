...
{
    private class RegularStringSolver : Puzzle<RegularStringSolver.ResultNames>, IPuzzle
    {
        ...
        {
            var profiles = new Dictionary<string, DateNotationComponents>();
            ...
            var names = ImmutableArray.CreateBuilder<string>(profiles.Count);
        ...
        private static DiaryEntry ParseDiaryEntry(SpanString line)
        {
            ...
            var names = namesSpan
                .SplitSelect(", ", ConstructString)
                .ToImmutableArray();
            return new(date, names);
        }

        private static string ConstructString(SpanString span)
        {
        ...
        public sealed record ResultNames(ImmutableArray<string> Names)
        {
            public override string ToString()
            {
                var names = Names.Order();
