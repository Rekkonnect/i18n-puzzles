...
partial class Season1Day9Benchmarks
{
    private class UmbraStringSolver : Puzzle<UmbraStringSolver.ResultNames>, IPuzzle
    {
        private static readonly Encoding _umbraStringEncoding =
#if UTF16_UMBRA_STRING
            Encoding.Unicode
#else
            Encoding.UTF8
        ...
        static PuzzleDayIdentifier IPuzzle.PuzzleDayIdentifier => new(01, 09);
        ...
        public void LoadInput(U8String bytes)
        {
            _input = bytes.Lines
                .Select(ParseDiaryEntryUtf8)
                .ToImmutableArray()
                ;
        }

        private static DiaryEntry ParseDiaryEntryUtf8(U8String line)
        {
            line = line.Trim();
            var (dateSpan, namesSpan) = line.SplitFirst((byte)':');
            dateSpan = dateSpan.Trim();
            namesSpan = namesSpan.Trim();
            var date = ParseDateNotationU8(dateSpan);
            var names = namesSpan
                .Split(", "u8)
                .SelectToImmutable(ConstructUmbraString);
        ...
        private static UmbraString ConstructUmbraString(SpanString span)
        {
            ...
            Span<byte> newBytes = bytes.Length > UmbraString.MaxShortLength
                ? new byte[bytes.Length]
                : stackalloc byte[bytes.Length]
                ;
            bytes.CopyTo(newBytes);
            return UmbraString.Construct(newBytes);
#else
        ...
        private static UmbraString ConstructUmbraString(U8String span)
        {
            return UmbraString.Construct(span.AsSpan());
        }
        ...
        private static DateNotation ParseDateNotationU8(U8String notation)
        {
            var (a, bc) = notation.SplitFirst('-');
            var (b, c) = bc.SplitFirst('-');
            int aValue = a.ParseInt32U8();
            int bValue = b.ParseInt32U8();
            int cValue = c.ParseInt32U8();
            return new(aValue, bValue, cValue);
        }
        ...
        public sealed record ResultNames(ImmutableArray<UmbraString> Names)
        {
            ...
            {
                var names = Names
                    .Select(s => s.ToString(_umbraStringEncoding))
                    .Order();
